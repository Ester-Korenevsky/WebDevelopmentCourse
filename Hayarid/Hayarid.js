...
lib.ssMetadata = [
		{name:"Hayarid_atlas_P_", frames: [[1757,390,147,72],[0,855,1087,753],[1282,0,357,265],[1420,267,94,82],[1089,855,796,868],[1795,264,35,36],[1282,340,56,63],[1757,264,36,37],[1757,317,151,71],[1641,0,390,108],[1641,264,114,138],[1870,110,125,205],[1641,110,227,152],[1282,267,136,71],[0,0,1280,853],[0,1610,947,665]]},
		{name:"Hayarid_atlas_NP_", frames: [[3204,0,420,682],[2442,2209,644,363],[2564,1007,900,1200],[3626,0,440,581],[0,0,1280,1920],[1282,1866,1158,767],[3204,684,479,282],[1282,0,1920,1005],[1282,1007,1280,857],[0,1922,1263,665]]}
];
...
(lib.Again = function() {
	this.spriteSheet = ss["Hayarid_atlas_P_"];
	this.gotoAndStop(0);
...
(lib.Altneuland = function() {
	this.spriteSheet = ss["Hayarid_atlas_NP_"];
	this.gotoAndStop(0);
}).prototype = p = new cjs.Sprite();
...
// stage content:
(lib.Hayarid = function(mode,startPosition,loop) {
	this.initialize(mode,startPosition,loop,{});
	...
	this.frame_0 = function() {
		...
		function init() { // שישמש בהמשך לקריאת הסאונד לבמה בעת הצלחה או אי הצלחה idסאונד במשחק ומתן 
			createjs.Sound.registerSound({
				id: "YES",
				src: "sounds/yesSound.mp3"
			});
			createjs.Sound.registerSound({
				id: "NO",
				src: "sounds/noSounds.mp3"
			});
		}
		var nextontage=false;
		function pressOdot() { //לחיצה על אודות
			...
			if (onstage == false&&endtable==false) { //אם זהו לא מסך הכניסה ולא מסך הסיכום
				for (var i = 0; i < myBtns[qnum].length; i++) { //הסרת האפשרות ללחוץ על הפחיות בזמן הצפייה באודות
					stage.getChildByName("btn" + i).removeEventListener('click', myClick);
				}
			}
			var txt = answers[qnum][rnd][rnd1];
			if (typeof txt != 'string'&&endtable==false) { //במידה והאודות מופעל בזמן המשחק והייתה תמונה בהגדלה היא תחזור לגודלה המקורי
		...
		function pressAxe() { ///איקס' סגירת אודות
			...
			toRemove("myodoti");
			toRemove("site");
			if (onstage == true) { //בדיקה אם זהו מסך הפתיחה
				componenet();
		
			}
			if (onstage == false&&endtable==false&&nextontage==false) { //אם זהו לא מסך הכניסה ולא מסך הסיכום ולא השהייה לפני לחיצה על המשך
				for (var i = 0; i < myBtns[qnum].length; i++) { //החזרת האפשרות ללחיצה על הפחית בזמן המשחק
		...
		var myBtns = [ //מערך לקטגוריות
			...
			["דוד בן גוריון", "זאב תאודור הרצל", "גולדה מאיר"]
		];
		...
		var endtable=false;//האם זהו מסך הסיכום
		var backfirst = new lib.backg();
		backfirst.name = "backf";
		backfirst.x = 925;
		backfirst.y = 0;
		stage.addChild(backfirst); //הוספת הרקע במסך הכניסה
		var onstage = true; //משתנה לבדיקה אם זהו המסך הראשי, בכל פעם שתמונת הכניסה מופיעה
		...
		stage.addChild(logo1);
			stage.getChildByName("logo").addEventListener('mouseover', myClick_mouseover);
		...
		componenet(); //פונקציה המכילה את העלאת הקומפוננט
		function componenet() {
		...
		function create2() { //פונקציה המפעילה את המשתנים שאינם משתנים מתחילת המשחק שנבחר ועד לסיום מענה המסיחים, הכוללת את הפחיות, הקטגוריות עליהם
			...
			stage.getChildByName("logo").addEventListener('mouseover', myClick_mouseover);
				stage.getChildByName("logo").addEventListener("click", home);//הוספת האפשרות לחזור למסך הבית בזמן המשחק
		...
		function myExit() { //פונקצית האיקס של התמונה המוגדלת
			var myBoard = stage.getChildByName("boardW2");
		...
		function myClick(event) { //פונצקיה המופעלת בכל לחיצה על קטגוריה נבחרת על הפחית
			...
			if (typeof txt != 'string') {
				myBoard.removeEventListener('click', zoom); //הסרת האפשרות להגדלת התמונה לאחר מענה
			...
			setTimeout(function () { //לאחר שנייה הסרת היד מהבמה והצגת כפתור המשך לשאלה הבאה
				...
				next.addEventListener('click', myNext);
				next.addEventListener('mouseover', myClick_mouseover);
		nextontage=true;//הכפתור על הבמה
				toRemove("handN");
			}, 1000);
		...
		nextontage=false;//הכפתור ירד מהבמה
			toRemove("correct");
			toRemove("false1");
			toRemove("timerEnd");
			if (typeof txt != 'string') {
		...
		function startOver() { //פונקציה להפעלה מחדש של אותו המשחק, איתחול על האלמנטים
			...
			toRemove("coca");
			toRemove("coca2");
			nextontage=false;//כפתור המשך לא על הבמה
		
		endtable=false;//יציאה ממסך הסיכום
		...
		function zoomtable(event) { //פונקציה למעבר עכבר על התמונה בטבלה
			...
			stage.addChildAt(event.currentTarget, stage.getNumChildren() - 1); //מיקום התמונה  כך שתהיה מעל האובייקטים האחרים
		...
		function home() { //חזרה למסך הפתיחה הראשי, לבחירת המשחק
			...
			toRemove("handN");
			toRemove("myodoti");
		stage.getChildByName("logo").removeEventListener('click', home);//בחזרה למסך הפתיחה הסרת האפשרות לחזור למסך הפתיחה שוב
	...
	this.instance.setTransform(910,-9,1,1,0,0,0,50,11);
	...
	this.instance_1 = new lib.backgroundOpacity();
	this.instance_1.parent = this;
	this.instance_1.setTransform(-10,2,1.5,1.646);

	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FFFFFF").ss(2,1,1).p("AAAAQIAAgf");
	this.shape.setTransform(850,131.8);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape},{t:this.instance_1},{t:this.stand},{t:this.what},{t:this.direc},{t:this.myCL},{t:this.timer1},{t:this.fill},{t:this.kav},{t:this.GameName},{t:this.board1},{t:this.flags},{t:this.instance}]}).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(470,274.5,970.5,620);
// library properties:
lib.properties = {
	width: 960,
	height: 590,
	...
	opacity: 1.00,
	manifest: [
		{src:"images/Hayarid_atlas_P_.png?1518260658923", id:"Hayarid_atlas_P_"},
		{src:"images/Hayarid_atlas_NP_.jpg?1518260658923", id:"Hayarid_atlas_NP_"},
		{src:"https://code.jquery.com/jquery-2.2.4.min.js?1518260659013", id:"lib/jquery-2.2.4.min.js"},
		{src:"components/sdk/anwidget.js?1518260659013", id:"sdk/anwidget.js"},
		{src:"components/ui/src/css.js?1518260659013", id:"an.CSS"},
		{src:"components/ui/src/combobox.js?1518260659013", id:"an.ComboBox"}
	],
	preloads: []
};
